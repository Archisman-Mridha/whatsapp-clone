...
import { PhoneIsAlreadyRegisteredError, RetryOTPVerificationError, ServerError, WrongPasswordError, configSchema } from "../../utils"
...
export class UsersService {
  ...
  constructor(
    ...
    this.twilioVerifyService= this.twilioService.client.verify.v2
  ...
  async signup(args: SignupArgs) {
    const { identifiers }=
      await this.usersRepository.insert(args)
        .catch((error: Error) => {
    ...
    if(identifiers.length > 0)
      this.logger.log(`New user created with id ${identifiers[0].id}`)
  }

  // throws : KafkaRetriableException
  async sendVerificationOTP(phone: string) {
    await this.twilioVerifyService.verifications.create({ to: phone, channel: "sms" })
      .catch(error => {
  ...
  async verifyAccount(args: VerifyAccountArgs) {
    const { status }=
      await this.twilioVerifyService.verificationChecks.create({ to: args.phone, code: args.otp })
        .catch(error => {
  ...
  async doesUserWithIdExist(id: string): Promise<boolean> {
    await this.usersRepository.findOneOrFail({ where: { id: parseInt(id) }, select: { id: true }})
      .catch(error => {
  ...
  async getIdOfUserWithPhone(phone: string): Promise<string> {
    const { id }=
      await this.usersRepository.findOneOrFail({ where: { phone }, select: { id: true }})
        .catch(error => {
